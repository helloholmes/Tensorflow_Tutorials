...
# drop out layer
# rate is 0.5 while training, 1.0 while testing
dropout = tf.placeholder(dtype = tf.bool, shape = None)
...
for i in range(5000):
	...
	_, loss_ = sess.run([train_op, loss], feed_dict = {x_: x_train, y_: y_train, dropout: True})

	if i % 50 == 0:
		accuracy_ = sess.run([accuracy], feed_dict = {x_: x_val, y_: y_val, dropout: False})
...
test_acc = sess.run([accuracy], feed_dict = {x_: x_test, y_:y_test, dropout: False})
